// Package cli assembles the musicbrainz command tree from the musicbrainz
...
// NewApp assembles the kit application from the musicbrainz domain. The
...
//
// To add a command, declare it in musicbrainz/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := musicbrainz.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(musicbrainz.Domain{}).Register(app)

...
describes what users get. Pick a format with `-o`, or let musicbrainz choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
musicbrainz <command> -o table   # aligned columns for reading
musicbrainz <command> -o jsonl   # one JSON object per line, for piping
musicbrainz <command> -o json    # a single JSON array
musicbrainz <command> -o csv     # spreadsheet friendly
musicbrainz <command> -o tsv     # tab-separated
musicbrainz <command> -o url     # just the URL column
musicbrainz <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
musicbrainz <command> --fields id,title,url
```
...
```bash
musicbrainz <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
musicbrainz <command>            # a table, because this is a terminal
musicbrainz <command> | wc -l    # JSONL, because this is a pipe
```
